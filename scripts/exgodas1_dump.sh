...
# This should be the only change needed for GODAS.
  export ARGOTANK=$TANK
  export DUPC=off
  $DUMP  $CTIME  $CRAD  tesac
...
until [ $FLX_DATE -gt $flux_end_date ]; do
  ...
  if test "$SENDCOM" = 'YES'
  then
    cp taux.$FLX_DATE $COMOUT
    cp tauy.$FLX_DATE $COMOUT
    cp senflx.$FLX_DATE $COMOUT
    cp latflx.$FLX_DATE $COMOUT
    cp prate.$FLX_DATE $COMOUT
    cp netlwflx.$FLX_DATE $COMOUT
    cp netswflx.$FLX_DATE $COMOUT
  fi
