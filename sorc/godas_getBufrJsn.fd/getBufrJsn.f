...
! USAGE:
!   INPUT FILES:
!     UNIT 11  - JASON BUFR DATA
!
!   OUTPUT FILES:
!     UNIT 51  - JASON DATA IN ASCII
...
!     LIBRARY:
!       W3LIB    - w3tagb, w3tage, errexit
!       BUFRLIB  - openbf, ireadmg, ireadsb, ufbget, ufbtab, closbf
!
!   EXIT STATES:
...
!
      ...
      integer, parameter :: lunin=11, mxts=11, imn=1, max_prt_ibad_orb=50
      integer :: iargc
      integer :: ireadmg, ireadsb, idate, iret
...
! various counters
      ...
      call w3tagb('GODAS_GETBUFRJSN',2006,0164,0164,'NP23')

!
! get date from command line
!
      if (iargc() .eq. 0) go to 910
      call getarg(1,str)
      read(str,'(i8)',err=910) kdate
      write(6,'(a,i10)')'process date: ',kdate
...
!
      ...
      if(ibad_date.ne.0)then
        write(6,'(a)') ' ************************************************' 
        write(6,*) ' WARNING:  ',ibad_date, 'WRONG DATES IN BUFR FILE.' 
        write(6,'(a)') ' ************************************************' 
      endif
!
      call w3tage('GODAS_GETBUFRJSN')
      call errexit(0)
!     call exit(0)

  910 continue
      ierr=91
      write(6,*)  'ERROR READING DATE FROM COMMAND LINE.  EXIT ',ierr
      go to 990
  920 continue
      ierr=92
