/* main program: mppnccombine
   programmer:  Hans Vahlenkampr  org: NOAA/GFDL        date: 1999-07-15

   program history log:
     2003-07-30  David W. Behringer

   input files:
     multiple netCDF time_mean.nc files, one from each preocessor of 
     GODAS analysis decomposition

   output file:
     single combined netCDF time_mean.nc file
...
struct fileinfo
  {
   ...
   FILE *ieeefid;  /* Pointer to an IEEE binary file */
...
int process_ncinfile(char *, unsigned char, int, char *, int *,
                     unsigned char);
int copy_nc_data(struct fileinfo *, int, unsigned char);
int process_ieeeinfile(char *, unsigned char, int, char *, int *,
                       unsigned char);
int copy_ieee_data(struct fileinfo *, int);
#if DEBUG==1
      ...
      for (a=0; a < nfiles; a++)
        {
         ...
         if (infileerror==2)
           infileerror=process_ieeeinfile(infilename,appendnc,outncfid,
                                          outfilename,&nfiles2,verbose);
     ...
     for (a=inputarg; a < argc; a++)
       {
        ...
        infileerror=process_ncinfile(argv[a],appendnc,outncfid,outfilename,
                                     &nfiles2,verbose);
        if (infileerror==2)
          infileerror=process_ieeeinfile(argv[a],appendnc,outncfid,
                                         outfilename,&nfiles2,verbose);
...
/* Print the usage message for ncmppcombine */
void usage()
  {
   ...
   printf("now, but IEEE binary will be supported in the future), containing chunks of a\n");
...
int process_ncinfile(char *ncname, unsigned char appendnc, int outncfid,
   ...
   *nfiles=nfiles2;
   ...
   ncinfileerror=copy_nc_data(&ncinfile,outncfid,appendnc);
...
int copy_nc_data(struct fileinfo *ncinfile, int outncfid,
                 unsigned char appendnc)
  {
   ...
   /* Loop over all the variables (and dimensions) */
   for (v=0; v < ncinfile->nvars; v++)
     {
      /* Copy a dimension's values */
      if ((dimid=ncdimid(ncinfile->ncfid,ncinfile->varname[v]))!=(-1))
        {
         /* Avoid multiple writes of non-decomposed dimensions */
         if (appendnc && ncinfile->dimend[dimid]==(-1)) continue;

         if ((values=malloc(nctypelen(ncinfile->datatype[v])*
                            ncinfile->dimsize[dimid]))==NULL)
           {
            fprintf(stderr,"Error: cannot allocate memory for dimension \"%s\"'s values!\n",
                    ncinfile->varname[v]);
            return(1);
           }
         instart[0]=0; count[0]=ncinfile->dimsize[dimid];
         if (ncvarget(ncinfile->ncfid,v,instart,count,values)==(-1))
           {
            fprintf(stderr,"Error: cannot read dimension \"%s\"'s values!\n",
                    ncinfile->varname[v]);
            return(1);
           }
         outstart[0]=ncinfile->dimstart[dimid]-1;
         if (ncvarput(outncfid,v,outstart,count,values)==(-1))
           {
            fprintf(stderr,"Error: cannot write dimension \"%s\"'s values!\n",
                    ncinfile->varname[v]);
            return(1);
           }
         free(values);
        }
      /* Copy a variable's values */
      else
        {
         /* Get read/write dimension sizes for the variable */
         nrecs=1; recsize=1; varrecdim=(-1);
...
/* Open an IEEE binary file and get some information about it */
int process_ieeeinfile(char *ieeename, unsigned char appendnc, int outncfid,
                       char *outncname, int *nfiles, unsigned char verbose)
  {
   struct fileinfo ieeeinfile;  /* Information about an input IEEE file */
   unsigned char recbuf[4096];
   char rectype[20];

   printf("IEEE input files are not currently supported!\n"); return(3);

   /* Open an input IEEE binary file */
   if ((ieeeinfile.ieeefid=fopen(ieeename,"rb"))==NULL) return(1);

   while (!feof(ieeeinfile.ieeefid))
     {
      if (fread((void *)recbuf,1,4096,ieeeinfile.ieeefid)!=4096) break;
      sscanf((char *)recbuf,"%s",rectype);
      if (!strcmp(rectype,"GLOBAL"))  /* Global attribute */
        {
         printf("GLOBAL\n");
        }
      else if (!strcmp(rectype,"AXIS"))  /* Dimension info */
        {
         printf("AXIS\n");
        }
      else if (!strcmp(rectype,"VARIABLE"))  /* Attributes for variables */
        {
         printf("VARIABLE\n");
        }
      else if (!strcmp(rectype,"FIELD"))  /* Variable info */
        {
         printf("FIELD\n");
        }
     }
   ...
   /* Done */
   fclose(ieeeinfile.ieeefid); return(0);
  }


/* Copy all the data values in an input IEEE file */
int copy_ieee_data(struct fileinfo *infile, int outncfid)
  {
   return(0);
  }
