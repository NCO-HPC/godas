...
!  use shmem_interface
  ...
  implicit none
  private
  character(len=256), private :: version='$Id: mpp.F90 100938 2019-06-17 17:50:05Z hyun-chul.lee@noaa.gov $'
...
#endif use_libSMA
#ifdef use_libMPI
        write( stdout,* ) ' PT001 at mpp.F90'
      call MPI_INIT(error)
        write( stdout,* ) ' PT002 at mpp.F90',error
      call MPI_COMM_RANK ( MPI_COMM_WORLD, pe,        error )
        write( stdout,* ) ' PT003 at mpp.F90',pe,error
      call MPI_COMM_SIZE ( MPI_COMM_WORLD, npes,      error )
        write( stdout,* ) ' PT004 at mpp.F90',npes,error
      call MPI_COMM_GROUP( MPI_COMM_WORLD, group_all, error )
        write( stdout,* ) ' PT005 at mpp.F90',group_all, error
#endif
