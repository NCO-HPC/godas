...
module mpp_io_mod
  ...
  implicit none
  private
  character(len=256), private :: version='$Id: mpp_io.F90 100938 2019-06-17 17:50:05Z hyun-chul.lee@noaa.gov $' !RCS ID
...
!close all open netCDF units
      do unit = maxunits+1,2*maxunits
         if( mpp_file(unit)%opened ) then
           error = NF_SYNC(mpp_file(unit)%ncid)
!          error = NF_CLOSE(mpp_file(unit)%ncid)
         endif
      end do
#endif
