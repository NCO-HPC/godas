...
!
!   OUTPUT FILES:
!     UNIT 51  - SALINITY PROFILE DATA FOR ASSIMILATION IN IEEE
...
!
      ...
      real, allocatable, dimension(:) :: xt, yt, zt, zw, dz, tt
      real stde, evr
      real, allocatable, dimension(:) :: sz, st
...
!
      ...
      read(80,rec=n) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
      csign0 = csign
      idate0 = idate
      iyear0 = iyear
      imon0 = imon
      ...
      do i=2,nrc
        read(82,'(a8,i4,2i2,i10)') csign,iyear,imon,iday,n
        read(80,rec=n) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
        if (csign .ne. csign0 .or. iyear .ne. iyear0  .or. imon .ne. imon0  .or. iday .ne. iday0) then
          do k=1,kax
            if (cnt(k) .gt. 0) then
          ...
          enddo
          write(83) iyear0,idate0,csign0,yp0,xp0,kts,(st(k),sz(k),k=1,kts)
          csign0 = csign
          iyear0 = iyear
          idate0 = idate
          imon0 = imon
...
!
      do n=1,nprf
        read (83, err=130) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
        ndxd(n) = iyear*100000000 + idate
        nprfl(n) = n
        do k=kt+1,kax
          tt(k) = mVal
        enddo
        write (80,rec=n) iyear,idate,csign,yp,xp,kt,tt,zt
      end do
!
      close (83)
!
...
!
      do n=1,nprf
        read (80,rec=nprfl(n)) iyear,idate,csign,yp,xp,kt,tt,zt
        write (83, err=140) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
      end do
!
      close(80)
      rewind(83)
...
!
      ...
      do while (.not. inbnds(xi,yj) .or. jday0 .lt. jdayS)
        read (83, end=10, err=130) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
...
!
      ...
      do k=kt+1,kax
        tt(k) = mVal
      enddo
      write (80,rec=nrc) iyear,idate,csign,xi,yj,kt,tt,zt
!
! continue loop on profile file
!
      do while (.true.)
        read (83, end=100, err=130) iyear,idate,csign,yp,xp,kt,(tt(k),zt(k),k=1,kt)
        write (cdate,'(i8.8)') idate
        read (cdate,'(4i2)') imon, iday, ihr, imin
...
!
          nobs = 0
          do n=1,nrc
            read (80,rec=nprfl(n)) iyear,idate,csign,xi,yj,kt,tt,zt
            write (cdate,'(i8.8)') idate
            read (cdate,'(4i2)') imon, iday, ihr, imin
            dow = weekDay(iyear, imon, iday)
...
!
          ...
          if (inbnds(xi,yj)) then
            ...
            ndxp(nrc) = ndxp(nrc)*1000000 + int(xi*1000.0)
            nprfl(nrc) = nrc
            write (80,rec=nrc) iyear0,idate0,csign0,xi,yj,kts,st,sz
          else
            nout = nout + 1
          end if
!
